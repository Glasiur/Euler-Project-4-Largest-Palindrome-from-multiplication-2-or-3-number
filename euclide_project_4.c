...
int iterate_positive_3_factors(int lim,int i,int j,int min_k_increment,int *max_pal,int enforce_order,int *iteration_count,int max_factor){
    for(int k=min_k_increment;k<=max_factor&&i*j*k<lim&&(enforce_order?k<=j:1);k++){
        ...
        if(product<=*max_pal)
            ...
            log_debug("%di %dj %dk %d product %d iteration_count ++ \n",i,j,k,product,*iteration_count);
            *max_pal=product;
        }
    }
...
int research1_optimized_two_factors(int lim,int max_factor ,int *iteration_count){
    int max_pal=0;
    ...
    log_debug("%d iteration_count1 \n",*iteration_count);
    log_result("%d best palindrome for lim=%d in research1\n",max_pal,lim);
    return max_pal;
}

int research2_optimized_three_factors(int lim,int max_factor, int *iteration_count){
    int max_pal=0;
...
int main(){
    int c=0;
    research1_optimized_two_factors(3773,99,&c);
    c=0;
    research2_optimized_three_factors(168861,99,&c);
    c=0;
    research1_optimized_two_factors(117711,999,&c);
    return 0;
}