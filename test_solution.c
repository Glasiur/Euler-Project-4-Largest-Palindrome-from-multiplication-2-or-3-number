...
int research_sure_two_factors(int lim,int max_factor,int *iteration_count){
    int max_pal=0;
...
int research_sure_three_factors(int lim, int max_factor, int *iteration_count) {
    int max_pal = 0;
    for (int i = max_factor; i > 0 && i * max_factor * max_factor > max_pal; i--) {
        (*iteration_count)++;
        for (int j = max_factor; j > i && i*j*max_factor > max_pal; j--) {
            (*iteration_count)++;
            for (int k = max_factor; k > j  && i*j*k > max_pal; k--) {
                (*iteration_count)++;
                int product = i * j * k;
                if (product <= lim && product > max_pal && is_palindrome(product)) {
                    max_pal = product;
                }
            }
        }
...
int get_palindrome_candidate(int lim,int max_factor,int search_strategy_id,int *iteration_count){
    ...
    switch(search_strategy_id){
        case 2:palindrome=research_optimized_three_factors_3(lim,max_factor,iteration_count);break;
        case 3:palindrome=research_optimized_three_factors_3(lim,max_factor,iteration_count);break;
        case 4:palindrome=research_sure_two_factors(lim,max_factor,iteration_count);break;
        case 6:palindrome=research_sure_three_factors(lim,max_factor,iteration_count);break;
    }
    return palindrome;
}
